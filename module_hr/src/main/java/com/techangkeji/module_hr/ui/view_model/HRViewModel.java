...
public class HRViewModel extends BaseViewModel {
    ...
    public ObservableField<HRAdapter> adapter = new ObservableField<>();
...
//        }
    ...
    public BindingCommand typeCommand = new BindingCommand(() -> {
        ...
        if (isBusiness){
            businessList.add("商办");
            businessList.add("商铺");
            businessList.add("写字楼");
            businessList.add("商住");
        }else {
            businessList.add("住宅");
            businessList.add("别墅");
        }
        new TypePopupwindow(context.get(),businessList).showPopupWindow(choiceView.get());
    });

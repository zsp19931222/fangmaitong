...
/**
 * description:
 * author:created by Andy on 2019/9/18 0018 15:17
 * email:zsp872126510@gmail.com
 */
public class QualificationAuthViewModel extends BaseViewModel {
    public ObservableField<String> company = new ObservableField<>("");
    public ObservableField<String> linkman = new ObservableField<>("");
    public ObservableField<String> linkmanPhone = new ObservableField<>("");
    public ObservableField<String> linkmanIDNum = new ObservableField<>("");
    public ObservableField<String> companyUrl = new ObservableField<>("");
    public ObservableField<String> forntUrl = new ObservableField<>("");
    public ObservableField<String> reverseUrl = new ObservableField<>("");
    public ObservableField<Context> context = new ObservableField<>();
    public int frontResultCode = 10086;
    public int reverseResultCode = 10087;
    public int companyResultCode = 10088;

    public QualificationAuthViewModel(@NonNull Application application) {
        super(application);
    }
    ...
    public BindingCommand submitCommand = new BindingCommand(() -> {
        if (IsNullUtil.getInstance().isEmpty(company.get()) || IsNullUtil.getInstance().isEmpty(linkman.get()) || IsNullUtil.getInstance().isEmpty(linkmanPhone.get()) || IsNullUtil.getInstance().isEmpty(linkmanIDNum.get()) || IsNullUtil.getInstance().isEmpty(companyUrl.get()) || IsNullUtil.getInstance().isEmpty(forntUrl.get()) || IsNullUtil.getInstance().isEmpty(reverseUrl.get())) {
    ...
    * description:公司图片
    * author: Andy
    * date: 2019/9/18 0018 15:46
    */
    public BindingCommand companyCommand = new BindingCommand(() -> new RxPermissions((FragmentActivity) context.get())
            .request(Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE)
            .subscribe(new io.reactivex.observers.DefaultObserver<Boolean>() {
                @Override
                public void onNext(Boolean aBoolean) {
                    if (aBoolean) {
                        openGallery(companyResultCode);
                    } else {
                        ToastUtil.errorToast(context.get(), GET_PERMISSIONS_FAILED, false);
                    }
                }

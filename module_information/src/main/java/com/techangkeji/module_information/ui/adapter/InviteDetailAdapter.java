...
public class InviteDetailAdapter extends BaseQuickAdapter<RecruitmentListEntity.DataBean.ContactUserBean, BaseViewHolder> {
    public InviteDetailAdapter(int layoutResId, @Nullable List<RecruitmentListEntity.DataBean.ContactUserBean> data) {
        super(layoutResId, data);
    }

    @Override
    protected void convert(BaseViewHolder helper, RecruitmentListEntity.DataBean.ContactUserBean item) {
        GlideLoadUtils.getInstance().glideLoad(helper.itemView.getContext(),item.getHeadUrl(),helper.getView(R.id.iv_hrda),0,45);
        helper.setText(R.id.tv_hrda_name,(String)item.getRealName());

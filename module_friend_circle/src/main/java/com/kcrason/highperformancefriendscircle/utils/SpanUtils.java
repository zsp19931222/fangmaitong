...
public class SpanUtils {
    ...
    public static SpannableStringBuilder makeSingleCommentSpan(Context context, String childUserName, long childUserId, String commentContent,int entityType,long entityId) {
        ...
        if (!TextUtils.isEmpty(childUserName)) {
            builder.setSpan(new TextClickSpan(context, childUserName, childUserId,entityType,entityId), 0, childUserName.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        return builder;
    }

    public static SpannableStringBuilder makeReplyCommentSpan(Context context, String parentUserName, String childUserName, long parentUserId, long childUserId, String commentContent,int entityType,long entityId) {
        ...
        if (!TextUtils.isEmpty(parentUserName)) {
            ...
            builder.setSpan(new TextClickSpan(context, parentUserName, parentUserId,entityType,entityId), 0, parentEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        if (!TextUtils.isEmpty(childUserName)) {
            int childStart = parentEnd + 2;
            int childEnd = childStart + childUserName.length();
            builder.setSpan(new TextClickSpan(context, childUserName, childUserId,entityType,entityId), childStart, childEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        return builder;
    }
    ...
    public static SpannableStringBuilder makePraiseSpan(Context context, List<PraiseBean> praiseBeans) {
        if (praiseBeans != null && praiseBeans.size() > 0) {
            ...
            for (int i = 0; i < praiseSize; i++) {
                ...
                builder.setSpan(new TextClickSpan(context, praiseUserName, praiseBean.getPraiseUserId()), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
